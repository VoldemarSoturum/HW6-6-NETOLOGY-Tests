...
#Задание «Исследуем: есть ли связь между продолжительностью курса и 
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# ++++++Создаём функцию чтобы потом обратится к ней из теста:
def get_courses_list():
	courses_list = []
	for course, mentor, duration in zip(courses, mentors, durations):
		course_dict = {"title":course, "mentors":mentor, "duration":duration}
		courses_list.append(course_dict)
	return courses_list  #++++++++++Возвращаем результат в переменную.
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
#+++++++++++++++++++++++++++++++++++++++++++++++
#++++Функция для теста, для анализа курса. Тело прежнее и не менялось.
...
# indexes_m (индексы для сортировки курсов по количеству преподавателей)
	#++++ Перенесли список значений для каждой из переменной из 
	# оригинального кода
	indexes_d = [index for duration, index in duration_index]
	indexes_m = [index for count, index in mcount_index]

	return {
        "duration_order": indexes_d,
        "mentors_order": indexes_m,
        "has_correlation": indexes_d == indexes_m
    		}	# Возвращаем словарь



# #++++++++++ Этот участок перенёс в: 
# # indexes_d = [index for duration, index in duration_index]
# # indexes_m = [index for count, index in mcount_index]

# # Допишите код ниже:
# for durations, index in duration_index:
# 	indexes_d.append(index)
# # Для indexes_m напишите аналогичный код самостоятельно:
# for mentors_count, index in mcount_index:
# 	indexes_m.append(index)
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++

if __name__ == "__main__":
    courses_data = get_courses_list()
    analysis = analyze_courses(courses_data)
...
# Допишите код ниже:
# 
#++++++ Пересобрал вывод под новые реалии.    
    print("Связь есть" if analysis["has_correlation"] else "Связи нет")
    print(f"Порядок курсов по длительности: {analysis['duration_order']}")
    print(f"Порядок курсов по количеству преподавателей: {analysis['mentors_order']}")
#==============Старый вывод
